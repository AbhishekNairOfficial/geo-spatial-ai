...
export function buildSystemPrompt(summary: DatasetSummary): string {
  ...
  const usZipBlock = isUsZip
    ? [
        ...
        "- For **top-N** questions (e.g. “top 20 ZIPs by returns”): set `highlightTopN` to N, set `highlightZipCodes` to [], set `highlightMetric` to the column to rank (e.g. N1, A00100), and set `geoFeatures` to `[]` — the server will fill ZCTA polygons from ingested data.",
        ...
        "- **Do not** put `geoFeatures` entries for US ZIPs: the server always supplies Census ZCTA outline polygons. Use `highlightTopN` and/or `highlightZipCodes` (5-digit) so the server can fill the map. Leave `geoFeatures` as `[]` for this geography.",
